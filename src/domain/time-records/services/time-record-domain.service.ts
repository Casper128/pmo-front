...
export class TimeRecordDomainService {
  ...
  buildApiBody(reg: TimeRecord): TimeRecordApiBody {
    ...
    return {
      ...
      tipoActividad: reg.tipoActividad || this.parameters.defaultFor('tipoActividad'),
      HoraInicio: `${y}-${mo}-${d}T${ih}:${im}:00.000Z`,
      HoraFin: `${y}-${mo}-${d}T${fh}:${fm}:00.000Z`,
      fechaInicio,
      causa: reg.causa || this.parameters.defaultFor('causa'),
      complejidad: reg.complejidad || this.parameters.defaultFor('complejidad'),
      impacto: reg.impacto || this.parameters.defaultFor('impacto'),
      equipo: reg.equipo || this.parameters.defaultFor('equipo'),
      modoActuacion: reg.modoActuacion || this.parameters.defaultFor('modoActuacion'),
      lenguaje: reg.lenguaje || this.parameters.defaultFor('lenguaje'),
      tipoHora: reg.tipoHora || this.parameters.defaultFor('tipoHora'),
      funcional: reg.funcional,
      prefijo: reg.prefijo || this.parameters.defaultFor('prefijo'),
      objetoRicef: reg.objetoRicef || '',
      unity: reg.unity || '',
      descripcionActividad: reg.desc,
      observacion: reg.observacion || reg.desc,
      categoria: reg.categoria || this.parameters.defaultFor('categoria'),
